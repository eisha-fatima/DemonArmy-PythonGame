...
def char_selection():
    ...
    global var, char_choice,  window_width, window_height
    var = 2
...
def settingbackground():
    global canvas, score, id, game,  window_width, window_height,lives_counter, char1_pic, score_pic, game_finish
    game_finish = False
    game= False
    winsound.PlaySound("Blank.wav", winsound.SND_ASYNC)
    animation_frames()
...
def playGame():
    global counter, counter_pause, e, player, char_choice, lives_counter
    lives_counter = 3
    counter_pause = 0
    counter = 0
    try:
...
def LoadGame():
    global player,score,e, game, counter_pause, list_pause, live_counter
    print(player)
    if player != 'Guest':
        print("I am her")
...
####-------------------------------------------
def ScoreCheck():
    global counter, game_finish, gameover_pic, just_resume
    if not game_finish:
        if counter < 10:
            adding_hostiles(1)
            hostile_movement()
...
mainmenu_pic, filename, boss_pic, pause_pic, leaderboard_pic, gameover_pic = 0,0,0,0,0,0
def pictures_settings(window_width, window_height):
    global leaderboard_pic, mainmenu_pic, filename, boss_pic, pause_pic,gameover_pic
    #print()
